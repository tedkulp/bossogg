...
static char *mp3_read (song_s *song, int *size)
{
  static int i;
  static char *ret;
  ...
  char buffer2[mp3->synth.pcm.length * 4];
  char *out_ptr = buffer2; //here
  char *out_buf_end = buffer2 + (mp3->synth.pcm.length * 4); //here
  ...
  for (i = 0; i < mp3->synth.pcm.length; i++) {
    ...
    *(out_ptr++) = sample >> 8;
    ...
    if (out_ptr == out_buf_end) {
      //printf ("playing\n");
      ret = (char *)malloc (sizeof (char *) * mp3->synth.pcm.length * 4);
      memcpy (ret, buffer2, mp3->synth.pcm.length * 4);  //here
      //bossao_play_chunk (song, buffer, BUF_SIZE);
      out_ptr = buffer2; //here
    }
  }

  *size = mp3->synth.pcm.length * 4;
  
  int frame_ret;
  while ((frame_ret = mp3_decode_frame (mp3)) == DECODE_CONT)
    ;

  if (frame_ret == DECODE_BREAK)
    ret = NULL;
