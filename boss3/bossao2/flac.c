						      ...
						      void *client_data)
{
  ...
  if (flac_buf != NULL)
    free (flac_buf);
  flac_buf = (char *)malloc (sizeof (buf));
  memcpy (flac_buf, bufc, sizeof (buf));
  //free (flac_buf);
  //flac_buf = NULL;
  flac_buf_size = sizeof (buf);
...
char *chunk_play_flac (song_s *song, int *size)
{
  FLAC__FileDecoder *decoder = (FLAC__FileDecoder *)song->songlib->flac;
  ...
  char *ret = (char *)malloc (sizeof (char *) * flac_buf_size);
  memcpy (ret, flac_buf, flac_buf_size);
  //printf ("playing %d\n", flac_buf_size);
  //if (flac_buf_size != 0) bossao_play_chunk (song, flac_buf, flac_buf_size);
  *size = flac_buf_size;
